...
class MissingCodecImplementationException(Exception):
    pass
...
class FFprobeMissingException(Exception):
    pass


class NoValidVideoFileException(Exception):
    pass
...
def generic_two_pass(
    ...
    first_pass = ffmpeg_base_command.format(
        inputfile=inputfile,
    ...
    second_pass = ffmpeg_base_command.format(
        inputfile=inputfile,
...
def h264_encoding(
    ...
    """ h264 encoding """
    codecsettings = "-preset slow -c:v h264 -profile:v {profile}".format(
        profile=profile
    )

    if passes == 1:
        return ffmpeg_base_command.format(
            inputfile=inputfile,
...
def vp9_encoding(
    ...
    """
    codecsettings = "-quality good -speed 3 -c:v vp9 -profile:v {profile}".format(
        profile=profile
    )
    if passes == 1:
...
def build_ffmpeg_command(pvs, output_dir):
    ...
    if shutil.which("ffmpeg") is None:
        raise FFmpegMissingError(
            "you do not have ffmpeg installed, please find a creative way for getting it started."
        )
    ...
    if required_codec not in implemented_codecs:
        raise MissingCodecImplementationException(
            "your selected codec {} is not implemented so far, add it or cry".format(
                required_codec
            )
        )
    ...
    basefilename = (
        ...
        + ".mkv"
    )  # out filename + container type (e,g. `Beauty-h264-baseline-3750kbps-60fps-720p.mkv`)

    output_filename = output_dir + "/" + basefilename

    logfile = (
        "./logs/" + output_filename.replace("/", "_").replace(".", "_") + "_log.log"
    )

    ff_command = implemented_codecs[required_codec](
        inputfile=pvs["src"],
...
def build_commands_to_convert_to_4k_and_lossless(
    processes_video_dir, lossless_output_dir
):
    ...
    for infile in utils.get_files(processes_video_dir):
        ...
        settings["outputfile"] = infile.replace(
            processes_video_dir, lossless_output_dir
        )
...
def analyze_video_file(filename):
    ...
    if shutil.which("ffprobe") is None:
        raise FFprobeMissingException(
            "you need to have ffprobe installed, please read README.md."
        )
